...
srs_error_t SrsGoApiRtcPlay::serve_http(ISrsHttpResponseWriter* w, ISrsHttpMessage* r, SrsRtcUserConfig* ruc)
{
    ...
    bool edge = _srs_config->get_vhost_is_edge(ruc->req_->vhost);

    if (rtc_enabled && edge) {
        rtc_enabled = false;
        srs_warn("disable WebRTC for edge vhost=%s", ruc->req_->vhost.c_str());
    }
    ...
    bool rtmp_to_rtc = _srs_config->get_rtc_from_rtmp(ruc->req_->vhost);
    if (rtmp_to_rtc && edge) {
        rtmp_to_rtc = false;
        srs_warn("disable RTMP to WebRTC for edge vhost=%s", ruc->req_->vhost.c_str());
    }
